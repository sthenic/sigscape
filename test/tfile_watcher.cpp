...
TEST(FileWatcher, WatchFile)
{
    ...
    LONGS_EQUAL(ADQR_EAGAIN, watcher.WaitForMessage(message, 0));

    /* Create the file. */
    std::ofstream ofs(PATH, std::ios::out);
    LONGS_EQUAL(ADQR_EOK, watcher.WaitForMessage(message, 300));
...
TEST(FileWatcher, WriteToFile)
{
    ...
    LONGS_EQUAL(ADQR_EOK, watcher.PushMessage({FileWatcherMessageId::UPDATE_FILE,
                                               std::make_shared<std::string>(CONTENTS0)}));

    LONGS_EQUAL(ADQR_EOK, watcher.WaitForMessage(message, 600));
    LONGS_EQUAL(FileWatcherMessageId::FILE_CREATED, message.id);
