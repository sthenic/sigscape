...
TEST(MessageThread, FailedStart)
{
    ...
    LONGS_EQUAL(0, thread.Start());

    struct Message msg;
    LONGS_EQUAL(0, thread.WaitForMessage(msg, 1000));
...
TEST(MessageThread, RevolvingMessages)
{
    ...
    struct Message msg;
    LONGS_EQUAL(0, thread.WaitForMessage(msg, 500));
    LONGS_EQUAL(MESSAGE_ID_IS_STARTED_OK, msg.id);
    LONGS_EQUAL(0, msg.code);
    ...
    for (int i = 0; i < NOF_MESSAGES; ++i)
    {
        ...
        LONGS_EQUAL(0, thread.PostMessage(msg));
    }

    /* Expect a timeout. */
    LONGS_EQUAL(-1, thread.WaitForMessage(msg, 500));
    thread.Stop();

    LONGS_EQUAL(0, thread.VerifyComplete());
}
